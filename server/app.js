...
app.get("/api/finished_goods", async (req, res) => {
  try {
    const result = await getPool().query("SELECT sku, name, collection, brand, work_order_id, product_image FROM finishedgoods");
    res.json(result.rows);
  } catch (err) {
    res.status(500).send(err.message);
  }
...
app.put('/api/work_orders/:id/complete', async (req, res) => {
  ...
  try {
    ...
    const woResult = await client.query(
      'SELECT sku, quantity_needed, artisan_id FROM workorder WHERE work_order_id = $1',
      [id]
    );
...
app.get("/api/audit_logs", async (req, res) => {
  try {
    const result = await getPool().query(`
      SELECT 
        l.log_id, l.user_id, l.action, l.role, l.timestamp, 
        COALESCE(
