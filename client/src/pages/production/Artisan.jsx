    ...
    <div className="w-full flex flex-col font-sans antialiased text-slate-900">
      <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 mb-8 flex gap-4 items-center">
        <div className="relative flex-1 group">
          ...
          <input type="text" placeholder="Search resources..." className="w-full bg-[#F8F9FA] border-none rounded-2xl py-3.5 pl-12 pr-4 outline-none font-bold text-slate-700" value={searchTerm} onChange={(e) => {setSearchTerm(e.target.value); setCurrentPage(0); setCurrentMatPage(0);}} />
        </div>
            ...
            {currentOrders.map((order) => (
              <div key={order.work_order_id} className="border border-gray-200 rounded-[2rem] p-4 bg-white shadow-sm flex flex-col hover:shadow-md transition-all text-left">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="w-14 h-14 rounded-2xl overflow-hidden border bg-slate-50 flex-shrink-0">
                    {order.product_image ? <img src={order.product_image} className="w-full h-full object-cover" /> : <HiPhoto size={20} className="m-auto mt-4 text-slate-200" />}
                  </div>
                </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-[100] p-6 text-left">
          <div className="bg-white rounded-[3rem] w-full max-w-4xl p-8 relative shadow-2xl max-h-[95vh] flex flex-col overflow-hidden">
            <div className="flex justify-between items-start mb-6">
              <div><h2 className="text-3xl font-black text-slate-900 leading-tight uppercase tracking-tighter">Work Order Form</h2><p className="text-slate-400 font-bold mt-1 tracking-tight text-xs">Resource allocation and production management.</p></div>
              <div className="flex items-center gap-4">
                ...
                <button onClick={() => {setShowWorkOrderModal(false); setWoImagePreview(null);}} className="text-slate-300 hover:text-black transition-all bg-slate-50 p-2 rounded-full shadow-sm"><HiXMark size={24}/></button>
              </div>
            </div>
            <form onSubmit={handleCreateWorkOrder} className="flex-1 flex flex-col min-h-0">
              <div className="grid grid-cols-2 gap-8 mb-6 overflow-y-auto pr-2 no-scrollbar">
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-3 text-left">
                    <div className="space-y-1"><label className="text-[9px] uppercase tracking-[0.2em] text-slate-400 ml-2 font-black text-left">Product SKU & Name</label><select required className="w-full bg-[#F3F4F6] rounded-xl p-3 outline-none border border-transparent font-bold text-xs" value={woForm.sku} onChange={e => setWoForm({...woForm, sku: e.target.value})}><option value="">Select Product...</option>{finishedGoods.map(fg => (<option key={fg.sku} value={fg.sku}>{fg.sku} - {fg.name}</option>))}</select></div>
                ...
                <div className="flex flex-col">
                  <div className="flex justify-center mb-4 flex-shrink-0">
                    <div className="w-28 h-28 rounded-[2rem] border-4 border-white shadow-xl flex items-center justify-center bg-slate-50 overflow-hidden relative group">
                      {woImagePreview ? <img src={woImagePreview} className="w-full h-full object-cover" /> : <HiPhoto size={40} className="text-slate-200" />}
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><p className="text-white text-[9px] font-black uppercase">Upload Design</p></div>
              ...
              <div className="flex justify-between items-center pt-4 border-t border-slate-100 mt-auto flex-shrink-0">
                ...
                <div className="flex gap-3">
                  <button type="button" onClick={() => {setShowWorkOrderModal(false); setWoImagePreview(null);}} className="px-8 py-3 border-2 border-slate-100 rounded-xl text-slate-400 uppercase text-[10px] font-black hover:bg-slate-50 transition-all">Cancel</button>
