    ...
    <div className={isStandalone ? "w-full h-full flex flex-col font-sans antialiased text-slate-900 overflow-hidden" : ""}>
        ...
        <div className="flex justify-between items-center mb-6 lg:mb-10 text-slate-900 px-1 lg:px-2 gap-3">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <div className="flex gap-1.5">
              <button onClick={() => setCurrentMatPage(p => Math.max(p - 1, 0))} disabled={currentMatPage === 0} className="p-1.5 rounded-full border disabled:opacity-30 transition-all hover:bg-slate-100"><HiChevronLeft size={16} /></button>
              <button onClick={() => setCurrentMatPage(p => p + 1)} disabled={currentMatPage >= Math.ceil(filteredMaterials.length / matsPerPage) - 1} className="p-1.5 rounded-full border disabled:opacity-30 transition-all hover:bg-slate-100"><HiChevronRight size={16} /></button>
            </div>
            <button
          ...
          <table className="w-full text-center border-separate border-spacing-y-2 min-w-[480px]">
            <thead>
              <tr className="text-[10px] font-black text-slate-300 uppercase tracking-widest border-b">
                ...
                <th className="pb-4 hidden md:table-cell">Unit</th>
                <th className="pb-4">Cost</th>
                <th className="pb-4">Status</th>
                <th className="pb-4 text-right pr-2 lg:pr-4">Actions</th>
              </tr>
            </thead>
              ...
              {currentMaterials.map((m) => (
                <tr key={m.material_id} className="hover:bg-slate-50 transition-all group">
                  <td className="py-3 pl-2 rounded-l-2xl border-y border-l border-transparent group-hover:border-slate-100">
                    <div className="w-10 h-10 lg:w-12 lg:h-12 mx-auto bg-slate-100 rounded-xl overflow-hidden flex items-center justify-center border border-slate-200">
                  ...
                  <td className="py-3 text-right pr-2 lg:pr-4 rounded-r-2xl border-y border-r border-transparent group-hover:border-slate-100">
                    <button
                      ...
                      className="p-2 lg:p-3 bg-white text-slate-300 hover:text-black hover:shadow-md rounded-xl transition-all border border-slate-100"
                    >
                      <HiPencil size={15} />
                    </button>
                  </td>
      ...
      {showMatModal && (
        <div className="fixed inset-0 flex justify-center items-center z-[100] p-4 lg:p-6 text-left backdrop-blur-md bg-black/10">
          <div className="bg-white rounded-[2rem] lg:rounded-[3rem] w-full max-w-2xl p-6 lg:p-12 relative shadow-2xl overflow-hidden max-h-[90vh] flex flex-col border border-slate-100">
            <button onClick={closeModal} className="absolute top-6 right-6 lg:top-10 lg:right-10 text-gray-400 hover:text-black transition-all bg-slate-50 p-2 rounded-full shadow-sm">
              <HiXMark size={22} />
            </button>
            <h2 className="text-2xl lg:text-4xl font-black text-slate-900 uppercase mb-6 lg:mb-8 tracking-tighter leading-none">
              {isUpdateMat ? 'Modify Material' : 'Register Material'}
            </h2>
            <form onSubmit={handleMatSubmit} className="space-y-5 overflow-y-auto no-scrollbar flex-1 pr-1">
              <div className="flex flex-col items-center mb-6 lg:mb-10 flex-shrink-0">
                <div className="w-24 h-24 lg:w-32 lg:h-32 rounded-[2rem] lg:rounded-[2.5rem] border-4 border-white shadow-xl flex items-center justify-center bg-slate-50 overflow-hidden relative group">
                  {imagePreview
