...
export default function Order() {
  ...
  const filteredOrders = orders.filter(order => {
    ...
    const matchesSearch =
      ...
      order.assignment_id?.toString().includes(sTerm) ||
  ...
  const handleReceiveOrder = async (id) => {
    if (!window.confirm("Mark this order as Received? This will add the quantity to Raw Materials stock.")) return;
    try {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!loggedInUserId) {
      alert("Error: Walang nahanap na user session. Mag-login ulit.");
      return;
    }
                    ...
                    <tr key={order.assignment_id} className="hover:bg-slate-50 transition-all group">
                      ...
                      <td className="py-4 text-right pr-4 rounded-r-2xl border-y border-r border-transparent group-hover:border-slate-100">
                        <div className="flex justify-end gap-2">
                          {!received && (
                            <button
                              onClick={() => handleReceiveOrder(order.assignment_id)}
                          ...
                          {received ? (
                            <div
                              className="p-3 rounded-2xl border border-slate-100 bg-slate-50 opacity-30 cursor-not-allowed"
                              title="Already received — cannot be edited"
                            >
                              <HiOutlinePencil size={18} className="text-slate-400" />
                            </div>
                          ) : (
                            <button
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex justify-center items-center z-[100] p-6">
          <div className="bg-white rounded-[3rem] w-full max-w-2xl p-12 relative shadow-2xl text-left">
            <button onClick={() => { setShowModal(false); resetForm(); }} className="absolute top-10 right-10 text-slate-300 hover:text-black transition-all bg-slate-50 p-2 rounded-full shadow-sm">
              <HiX size={28} />
            </button>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-2 gap-6 font-bold">
                ...
                <div className="space-y-2">
                  ...
                  <select
                    required
                    className="w-full bg-[#F3F4F6] rounded-2xl p-4 outline-none border border-transparent font-bold text-sm"
              ...
              <div className="flex gap-4 pt-4 justify-end">
                ...
                <button type="submit" className="px-12 py-4 bg-black text-white rounded-2xl uppercase text-[11px] font-black shadow-xl transition-all hover:bg-stone-800 tracking-widest">
                  {isEdit ? "Save Changes" : "Confirm PO"}
                </button>
              </div>
            </form>
          </div>
