...
export default function Warehouse() {
  ...
  return (
    <div className="w-full flex flex-col font-sans antialiased text-slate-900">
      <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 mb-3 flex gap-3 items-center">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
        {[
          { label: "Warehouses", val: stats.total, sub: "Storage Locations", icon: <HiOutlineBuildingStorefront size={18} /> },
          { label: "Total Capacity", val: `${stats.totalCapacity.toLocaleString()} Units`, sub: "Max Storage", icon: <HiOutlineCube size={18} /> },
          { label: "Total Stocks", val: `${stats.totalStocks.toLocaleString()} Units`, sub: "Across All", icon: <HiOutlineArchiveBox size={18} /> },
          { label: "Low Stock", val: stats.totalLowStock, sub: "Needs Attention", icon: <HiOutlineCube size={18} /> },
        ].map((item, i) => (
          <div key={i} className="bg-white p-4 rounded-[1.5rem] border border-gray-100 shadow-sm flex flex-col justify-between">
            <div className="flex justify-between items-start">
              <p className="text-gray-400 text-[9px] font-black uppercase tracking-widest leading-tight">{item.label}</p>
              <span className="text-slate-300 flex-shrink-0">{item.icon}</span>
            </div>
            <div className="mt-2">
              <h2 className="text-lg lg:text-2xl font-black text-slate-900 tracking-tight truncate">{item.val}</h2>
      ...
      <div className="space-y-3 pb-10">
        <section className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-4 lg:p-6 relative h-fit text-left">
          <div className="flex justify-between items-center mb-4 lg:mb-6 px-1 lg:px-2">
            <div>
              <h1 className="text-xl lg:text-2xl font-black uppercase text-slate-900 leading-none tracking-tighter">
                Manage Warehouses
              </h1>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="bg-black text-white px-3 lg:px-5 py-2 lg:py-2.5 rounded-xl text-[9px] font-black uppercase shadow-lg hover:scale-105 transition-all tracking-widest whitespace-nowrap"
              >
                + Add
              </button>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-5">
            {currentWarehouses.length > 0 ? (
              currentWarehouses.map(w => {
                ...
                return (
                  <div
                    ...
                    className="border border-gray-100 rounded-2xl lg:rounded-3xl p-4 lg:p-5 bg-white shadow-sm flex flex-col hover:shadow-md transition-all relative cursor-pointer"
                  >
                    <div className="flex justify-between items-start mb-3">
                      <div className="w-8 h-8 lg:w-10 lg:h-10 bg-slate-50 rounded-xl flex items-center justify-center border border-slate-100 text-slate-400">
                      ...
                      <div className="relative" onClick={e => e.stopPropagation()}>
                        ...
                        {activeMenuId === w.warehouse_id && (
                          <div className="absolute right-0 mt-1 w-24 bg-white border border-slate-100 rounded-xl shadow-xl z-20 py-1.5 text-left">
                            <button
                              onClick={() => {
                    ...
                    <div className="mb-2 overflow-hidden">
                      <h3
                        className="font-black text-sm lg:text-base text-slate-900 leading-tight truncate overflow-hidden whitespace-nowrap w-full"
                        title={w.name}
                      >
                        {w.name}
                      </h3>
                    ...
                    <div className="space-y-1 mb-3">
                      <div className="flex items-center gap-1.5 overflow-hidden">
                        <HiOutlineMapPin size={10} className="text-slate-300 flex-shrink-0" />
                        <span className="text-[9px] font-bold text-slate-500 truncate">{w.location || "No Location"}</span>
                      </div>
                      <div className="flex items-center gap-1.5 overflow-hidden">
                        <HiOutlineUser size={10} className="text-slate-300 flex-shrink-0" />
                        <span className="text-[9px] font-bold text-slate-500 truncate">{w.manager_name || "Unassigned"}</span>
                      </div>
                    </div>

                    <div className="bg-slate-50 rounded-xl lg:rounded-2xl p-2.5 border border-slate-100 mt-auto">
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[8px] font-black uppercase text-slate-400 tracking-wider">Stock vs Cap</span>
                      ...
                      <div className="flex justify-between mt-1">
                        <span className="text-[8px] text-slate-400 font-bold">{getWarehouseStock(w.name).toLocaleString()}</span>
                        <span className="text-[8px] text-slate-400 font-bold">{Number(w.capacity_total || 0).toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                );
              ...
              <div className="col-span-2 lg:col-span-4 py-16 flex flex-col items-center justify-center text-slate-300 border-2 border-dashed border-slate-100 rounded-3xl bg-slate-50/50">
                <HiOutlineSearch size={36} className="mb-2 opacity-20" />
      ...
      {showInventoryModal && inventoryWarehouse && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex justify-center items-center z-[100] p-4 lg:p-6 text-left">
          <div className="bg-white rounded-[2rem] lg:rounded-[3rem] w-full max-w-3xl p-6 lg:p-10 relative shadow-2xl flex flex-col max-h-[90vh]">
            <button
            ...
            <div className="mb-4">
              ...
              <div className="flex items-center gap-3 mt-2">
                <p className="flex items-center gap-1.5 text-[9px] font-bold text-slate-400">
                  <HiOutlineMapPin size={10} className="text-slate-300" />
                  {inventoryWarehouse.location || "No Location"}
                </p>
                <span className="text-slate-200">•</span>
                <p className="flex items-center gap-1.5 text-[9px] font-bold text-slate-400">
                  <HiOutlineUser size={10} className="text-slate-300" />
                  ...
                  <table className="w-full border-separate border-spacing-y-2 min-w-[480px]">
                    <thead className="sticky top-0 bg-white z-10">
                      <tr className="text-[10px] font-black text-slate-300 uppercase tracking-widest">
                        <th className="pb-2 text-left pl-2">Product</th>
                        <th className="pb-2 text-center hidden sm:table-cell">Category</th>
                    ...
                    <tbody className="font-bold text-slate-700">
                      {filteredInventory.map(item => {
                        ...
                        const isLow = stock <= 0 ? "NO STOCK" : stock <= min ? "LOW" : "OK";
                        const statusColor = isLow === "OK" ? "bg-emerald-500 text-white" : "bg-rose-500 text-white";
                        return (
                          <tr key={item.sku} className="group hover:bg-slate-50 transition-all">
                            <td className="py-2.5 pl-2 rounded-l-2xl border-y border-l border-transparent group-hover:border-slate-100 max-w-[160px]">
                              <div className="flex items-center gap-2 min-w-0">
                                <div className="w-8 h-8 bg-slate-100 rounded-xl overflow-hidden flex items-center justify-center border border-slate-200 flex-shrink-0">
                                  {item.product_image
                                ...
                                <div className="min-w-0">
                                  <span className="text-[10px] font-black text-slate-900 uppercase truncate block" title={item.name}>
                                    {item.name || "Unnamed"}
                                  </span>
                                  <span className="text-[9px] text-slate-400 font-bold truncate block" title={item.sku}>
                                    {item.sku}
                                  </span>
                                </div>
                              </div>
                            </td>
                            <td className="py-2.5 text-center border-y border-transparent group-hover:border-slate-100 hidden sm:table-cell">
                              <span className="text-[10px] text-slate-500 font-bold uppercase">{item.category || "---"}</span>
                            </td>
                            <td className="py-2.5 text-center font-black text-slate-900 border-y border-transparent group-hover:border-slate-100 text-xs">
                              {item.current_stock || 0}
                            </td>
                            <td className="py-2.5 text-center text-[10px] text-slate-400 font-bold border-y border-transparent group-hover:border-slate-100">
                              {item.min_stocks || 0}
                            </td>
                            <td className="py-2.5 text-center font-black text-emerald-600 border-y border-transparent group-hover:border-slate-100 whitespace-nowrap text-xs">
                              ₱{parseFloat(item.selling_price || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                            </td>
                            <td className="py-2.5 text-center rounded-r-2xl border-y border-r border-transparent group-hover:border-slate-100">
                              <span className={`px-2.5 py-1 rounded-full text-[8px] font-black uppercase ${statusColor}`}>
                                {isLow}
                              </span>
                            </td>
                          </tr>
