    ...
    <div className="w-full flex flex-col font-sans antialiased text-slate-900 overflow-hidden">
      <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 mb-3 flex gap-4 items-center">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search Supplier..." className="w-full bg-[#F8F9FA] border-none rounded-xl py-2.5 pl-11 pr-4 outline-none font-bold text-slate-700 text-xs" value={searchTerm} onChange={(e) => {setSearchTerm(e.target.value); setCurrentPage(0);}} />
        </div>
      </div>
      ...
      <div className="space-y-3 pb-10">
          ...
          <div className="flex justify-between items-center mb-6 px-2">
            ...
            <div className="flex items-center gap-3">
              <div className="flex gap-1 mr-2">
                ...
                <button onClick={() => setCurrentPage(p => p + 1)} disabled={currentPage >= Math.ceil(sortedAndFilteredSuppliers.length / itemsPerPage) - 1} className="p-1.5 rounded-full border disabled:opacity-30 hover:bg-slate-100"><HiChevronRight size={16}/></button>
              </div>
        ...
        <section className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-8 h-fit text-left">
          <div className="flex justify-between items-center mb-3 px-2">
            ...
            <div className="flex gap-1 mr-2">
              <button onClick={() => setProcurementPage(p => Math.max(p - 1, 0))} disabled={procurementPage === 0} className="p-1.5 rounded-full border disabled:opacity-30 hover:bg-slate-100"><HiChevronLeft size={16}/></button>
              <button onClick={() => setProcurementPage(p => p + 1)} disabled={procurementPage >= Math.ceil(orders.length / procurementPerPage) - 1} className="p-1.5 rounded-full border disabled:opacity-30 hover:bg-slate-100"><HiChevronRight size={16}/></button>
            </div>
          </div>
          <div className="overflow-hidden">
            <table className="w-full text-center border-separate border-spacing-y-1">
              <thead>
                <tr className="text-[9px] font-black text-slate-300 uppercase tracking-widest border-b">
                  <th className="pb-3 text-left pl-6 w-[15%]">Order ID</th>
                  <th className="pb-3 text-left w-[18%]">Supplier</th>
                  <th className="pb-3 text-left w-[20%]">Material</th>
                  <th className="pb-3 text-left w-[12%]">Qty</th>
                  <th className="pb-3 text-left w-[18%]">Value</th>
                  <th className="pb-3 text-left w-[17%]">Status</th>
                </tr>
              </thead>
              <tbody className="font-bold text-slate-700">
                {currentOrders.map(order => (
                  <tr key={order.purchase_order_id} className="hover:bg-slate-50 transition-all group">
                    <td className="py-3 pl-6 rounded-l-2xl border-y border-l border-transparent group-hover:border-slate-100 text-slate-900 font-black text-xs text-left">PO-{order.purchase_order_id}</td>
                    <td className="py-3 border-y border-transparent group-hover:border-slate-100 text-left truncate max-w-[120px] text-xs" title={order.supplier_name}>{order.supplier_name}</td>
                    <td className="py-3 border-y border-transparent group-hover:border-slate-100 text-left truncate max-w-[150px] text-[10px]" title={order.material_name}>{order.material_name}</td>
                    <td className="py-3 border-y border-transparent group-hover:border-slate-100 text-left text-slate-400 font-black text-xs">{order.ordered_quantity}</td>
                    <td className="py-3 border-y border-transparent group-hover:border-slate-100 text-left font-black text-emerald-600 text-xs">₱{Number(order.total_amount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                    <td className="py-3 pr-4 rounded-r-2xl border-y border-r border-transparent group-hover:border-slate-100 text-left">
              ...
              <table className="w-full text-left border-separate border-spacing-0">
                <thead className="sticky top-0 bg-white z-10">
                  <tr className="text-[10px] font-black text-slate-300 uppercase tracking-widest border-b">
                    <th className="pb-4 text-left pl-2">PO ID</th><th className="pb-4 text-left">Date</th><th className="pb-4 text-left">Material</th><th className="pb-4 text-center">Qty</th><th className="pb-4 text-right pr-6">Total</th><th className="pb-4 text-center">Status</th>
                  </tr>
                </thead>
                <tbody className="text-[11px] font-bold text-gray-700">
                  {supplierHistory.map(order => (
                    <tr key={order.purchase_order_id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="text-slate-400">{new Date(order.order_date).toLocaleDateString()}</td>
      ...
      {(showAddModal || showUpdateModal) && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex justify-center items-center z-[100] p-6 text-left">
          <div className="bg-white rounded-[3rem] w-full max-w-xl p-12 relative shadow-2xl animate-in zoom-in duration-300">
            ...
            <h2 className="text-4xl font-black text-slate-900 uppercase mb-8 tracking-tighter leading-none">{showAddModal ? "New Partner" : "Update Records"}</h2>
